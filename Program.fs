...
let Errors (x:string) correct =
    ...
    | (true, value) -> value :: correct  //добавляем число в acc []
    | _ -> 
...
and SearchElem listElem = 
    ...
    | (true, value) -> 
        ...
        let rec Search elem list index correct =
            ...
            | h::t -> 
                if h = elem then 
                    Search elem t (index + 1) (index :: correct)    //увеличиваем индекс и добавляем текущий индекс к acc
                else 
                     Search elem t (index + 1) correct //поиск без добавления индекса

        //рекурсивная функция начало с индекса 1 и пустого acc
...
let rec input3 () =
    ...
    let listElem =
        ...
        |> Array.fold (fun correct x -> Errors x correct) []   //для каждого элемента вызываем функцию проверки на корректность acc = []
